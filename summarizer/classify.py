...
#   4) --oFile Output file name to write classified text. If parameter --toFile is True
...
if __name__ == "__main__":
    ...
    if options.modelName.find('.SVM'):
        classifier = SVC()
    classifier = joblib.load(os.path.join(options.modelPath, 'models', options.modelName + '.sav'), mmap_mode=None)
    ...
    #print(classifier.predict_proba(listPredictedClasses))
    ...
    with open(os.path.join(options.outputPath, options.outputFile), "w", encoding="utf-8") as oFile:
        oFile.write('FEATURE_TEXT\tPREDICTED_CLASS\n')
        for ft,pc in zip(listFeatureTexts, listPredictedClasses):
