...
if __name__ == "__main__":
    ...
    tags = ["CC","CD","DT","EX","FW","IN","JJ","JJR","JJS","LS","MD","NN","NNS","NNP","NNPS","PDT","POS","PRP","RB","RBR","RBS","RP","SYM","TO","UH","VB","VBD","VBG","VBN","VBP","VBZ","WDT","WP","WRB"]
                    ...
                    with open(os.path.join(options.transformedPath, file.replace('term.txt', 'tra.txt')), "w", encoding="utf-8") as transformedFile:
                        for line in iFile:
                            if line == '\n':
                                ...
                                if re.search("((\-|\d)*[NACTG]+(\-|\d)*[NACTG]+(\-|\d)*){5,}", text, re.IGNORECASE):
                                    textTransformed = textTransformed + text + '|nucleotides|SSA '
                                else:
